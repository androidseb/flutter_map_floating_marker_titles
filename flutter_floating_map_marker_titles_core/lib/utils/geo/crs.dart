...
class SphericalMercator extends Projection {
  ...
  LatLng unproject(CustomPoint point) {
    var d = 180 / Math.pi;
    return LatLng(
        inclusiveLat((2 * Math.atan(Math.exp(point.y / r)) - (Math.pi / 2)) * d), inclusiveLng(point.x * d / r));
  }
}
...
class _Proj4Projection extends Projection {
  ...
  @override
  CustomPoint project(LatLng latlng) {
    var point = epsg4326.transform(proj4Projection, proj4.Point(x: latlng.longitude, y: latlng.latitude));
  ...
  @override
  LatLng unproject(CustomPoint point) {
    var point2 = proj4Projection.transform(epsg4326, proj4.Point(x: point.x, y: point.y));
